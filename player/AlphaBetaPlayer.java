...
public class AlphaBetaPlayer extends GGPlayer {
	...
	private Move computeBestMove(StateMachine machine, Role role, MachineState state)
		...
		for (int i = 0; i < legalMoves.size(); i++) {
			...
			int result = computeMinScore(role, state, machine, nextMove, 0, 100);
	 ...
	 */
	private int computeMinScore(Role role, MachineState state, StateMachine machine, Move move, int alpha, int beta)
		...
		for (int i = 0; i < legalJoints.size(); i++) {
			...
			int result = computeMaxScore(role, newState, machine, alpha, beta);
	 ...
	 * Returns the best score possible from the given possible moves
	 */
	private int computeMaxScore(Role role, MachineState state, StateMachine machine, int alpha, int beta)
			throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
		...
		List<Move> legalMoves = findLegals(role, state, machine);		// All legal moves from current state
		for (int i = 0; i < legalMoves.size(); i++) {
			int result = computeMinScore(role, state, machine, legalMoves.get(i), alpha, beta);
